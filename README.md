...
| `Remainder` | `string` | 去除省/市/区县后的剩余文本（可能为空） |
...
JSON 序列化字段名：`code`、`province`、`city`、`county`、`remainder`。
...
// 去除行政区划后的剩余文本
p.Parse("广东省深圳市鹏城俱乐部")
// → Code: "440300", City: "深圳市", Remainder: "鹏城俱乐部"

// Remainder 会全文擦除命中的行政名（不限定位置）
p.Parse("星海沈阳国际象棋俱乐部")
// → Code: "210100", City: "沈阳市", Remainder: "星海国际象棋俱乐部"
p.Parse("星海（沈阳）国际象棋俱乐部")
// → Code: "210100", City: "沈阳市", Remainder: "星海（）国际象棋俱乐部"
```
